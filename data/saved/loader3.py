...
# crate table 
session.execute("CREATE TABLE airlines4 (year text,month text,day text,dayofweek text, deptime text, crsdeptime text, arrtime text, crsarrtime text, uniquecarrier text, flightnum text, tailnum text, actualelapsedtime text, crselapsedtime text, airtime text, arrdelay text, depdelay text, origin text, dest text, distance text, taxiin text, taxiout text, cancelled text, cancellationcode text, diverted text, carrierdelay text, weatherdelay text, nasdelay text, securitydelay text, lateaircraftdelay text, PRIMARY KEY ((year, month), uniquecarrier, flightnum));")

insert_stmt = session.prepare("INSERT INTO airlines4 (year, month, day, dayofweek, deptime, crsdeptime, arrtime, crsarrtime, uniquecarrier, flightnum, tailnum, \
